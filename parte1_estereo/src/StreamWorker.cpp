...
void streamCamera(const std::string &url, CameraStream &stream) {
  ...
  std::thread processThread([&]() {
    while (running) {
      ...
      auto soi = std::search(localBuffer.begin(), localBuffer.end(), JPEG_SOI,
                             JPEG_SOI + 2);
      ...
      auto eoi =
          std::search(soi + 2, localBuffer.end(), JPEG_EOI, JPEG_EOI + 2);
      ...
      if (!img.empty()) {
        ...
        stream.frameCount++;
        double elapsed =
            std::chrono::duration<double>(now - stream.lastFpsTime).count();
